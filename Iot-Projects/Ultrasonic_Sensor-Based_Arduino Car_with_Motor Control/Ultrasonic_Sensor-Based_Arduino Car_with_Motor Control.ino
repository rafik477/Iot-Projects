...
void loop() {
  ...
  if (distance > 50) {
    ...
    turnLeft();     // Turn left if distance is between 30 cm and 50 cm
  } else if (distance > 10 && distance <= 30) {
    turnRight();    // Turn right if distance is between 10 cm and 30 cm
  } else {
    stopMotors();   // Stop if distance is 10 cm or less
  }
}
